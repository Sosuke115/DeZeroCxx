...
// FIXME: ここはオーバーロードが良さそう
template <typename T>
inline NdArrPtr as_array(const T& obj) {
  if constexpr (std::is_same_v<T, NdArrPtr>) {
    return obj;
  } else if constexpr (std::is_same_v<T, nc::NdArray<double>>) {
...
template <typename T>
inline VarPtr as_variable(const T& obj) {
  if constexpr (std::is_same_v<T, VarPtr>) {
    return obj;
  } else if (std::is_same_v<T, NdArrPtr>) {
...
class Variable {
 public:
  ...
  void backward(const bool retain_grad = true);
  void cleargrad();
  int generation = 0;
...
class Function : public std::enable_shared_from_this<Function> {
 public:
  ...
  virtual std::vector<NdArrPtr> backward(const std::vector<NdArrPtr>& gy) = 0;
};

class Add : public Function {
 public:
  std::vector<NdArrPtr> forward(const std::vector<NdArrPtr>& xs) override;
  std::vector<NdArrPtr> backward(const std::vector<NdArrPtr>& gy) override;
};
